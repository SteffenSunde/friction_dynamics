...
def main():
    ...
    files = glob.glob("data/del/P1200/history_single*del*")
    ...
    for i, df in enumerate(dataframes):
        ax2.plot(df.x, df.v, label=r"$\delta={:.2f}$".format(float(metas[i]["del"])), linestyle="-", linewidth=0.75)
    ...
    for i, df in enumerate(dataframes):
        ax3.plot(df.t, df.slip, label=r"$\delta={:.2f}$".format(float(metas[i]["del"])))
